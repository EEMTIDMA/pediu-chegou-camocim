 ...
 const gerenciarStatus = async (id, status) => {
   ...
   if (status === 'entregue') {
     ...
     await updateDoc(doc(db, 'perfil_entregador', user.uid), {
       ...
       creditosPrioridade:
         (perfil.entregasConcluidas + 1) % 10 === 0
           ? increment(1)
           : increment(0),
     });
   }
       ...
       <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
         ...
         <div>
           <p style={{ fontSize: '12px', margin: 0, fontWeight: 'bold' }}>
             {perfil.nome.split(' ')[0]}{' '}
             {perfil.creditosPrioridade > 0 && (
               <span style={{ color: CorDestaque }}>
                 ★{perfil.creditosPrioridade}
               </span>
             )}
           </p>
           <button
         ...
         <section>
           ...
           {pedidos
             .filter((p) => {
               ...
               const segundos =
                 (Date.now() -
                   (p.horaSolicitacao?.seconds * 1000 || Date.now())) /
                 1000;
               return (
                 !p.entregadorId &&
                 perfil.gpsAtivo &&
                 p.status === 'pendente' &&
                 dist <= (p.raioBusca || 1) &&
                 segundos >= (perfil.creditosPrioridade > 0 ? -10 : 0)
               );
             })
             .map((p) => (
               <div
           ...
           {entreguesFiltradas.map((p) => (
             ...
             >
               ...
               >
                 ...
                 >
                   <span>Produto:</span>{' '}
                   <b>R$ {parseFloat(p.valorProduto || 0).toFixed(2)}</b>
                 </div>
                 <div
                   style={{ display: 'flex', justifyContent: 'space-between' }}
                 >
         ...
         >
           <button
             ...
             style={{
               width: '100%',
               padding: '15px',
               background: perfil.lat && perfil.lng ? '#2ecc71' : CorDestaque,
               color: perfil.lat && perfil.lng ? '#FFF' : CorPrincipal,
               border: 'none',
               borderRadius: '12px',
               fontWeight: '900',
               marginBottom: '20px',
           ...
           <div style={{ display: 'grid', gap: '10px' }}>
             ...
             <input
               ...
               onChange={(e) =>
                 setPerfil({ ...perfil, telefone: maskPhone(e.target.value) })
               }
               style={{
                 padding: '10px',
                 border: '1px solid #EEE',
             ...
             <input
               ...
               value={perfil.cpf}
               onChange={(e) =>
                 setPerfil({ ...perfil, cpf: maskCPF(e.target.value) })
               }
               style={{
                 padding: '10px',
                 border: '1px solid #EEE',
